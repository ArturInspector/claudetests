...
""".strip()
    ...
    async def calculate_blind_zones(
        ...
        """
        Вычислить blind zones — области знаний которые пользователь упустил.
        
        Blind zones включают:
        1. Unexplored branches — связанные концепты которые не изучались
        2. Weak connections — известные концепты с низким mastery
        3. Missing prerequisites — концепты требующие знания которых нет
        
        Args:
            user_id: UUID пользователя
            topic: Фильтр по теме (опционально)
            min_mastery_threshold: Минимальный уровень для "освоено" (0.7)
            
        Returns:
            Dict с blind zones разных типов
        """
        try:
            async with self._session() as session:
                # 1. Unexplored branches
                unexplored = await self._find_unexplored_branches(
                    session, user_id, topic, min_mastery_threshold
                )
                
                # 2. Weak connections
                weak = await self._find_weak_connections(
                    session, user_id, topic, min_mastery_threshold
                )
    ...
    async def _find_unexplored_branches(
        self,
        ...
        """
        Найти концепты связанные с освоенными, но сами не изученные.
        
        Логика: если пользователь знает концепт A (mastery > threshold),
        и A связан с B, но пользователь не знает B — это blind zone.
        """
    ...
    async def _find_weak_connections(
        self,
        session: AsyncSession,
        user_id: str,
        topic: str | None,
        ...
        """
        Найти концепты которые пользователь знает, но слабо (mastery < threshold).
        
        Это области где нужно углубить понимание.
        """
    ...
    async def _find_missing_prerequisites(
        self,
        session: AsyncSession,
        user_id: str,
        topic: str | None,
        ...
        """
        Найти концепты с невыполненными prerequisites.
        
        Если концепт B требует понимания A (PREREQUISITE_FOR),
        но пользователь не знает A достаточно — это проблема.
        """
